...
class KnowledgeOrganizationManager {
    ...
    async renderSimpleOrganizationTable(dv, fieldName = 'domain-category', currentSubject, showDefinitions = null, debug = false) {
        ...
        categories.forEach(cat => {
            ...
            // Add row with accordion with toggle functionality on both header and details
            const rowId = `category-row-${cat.replace(/[^a-zA-Z0-9]/g, '-')}`;
            const detailsId = `category-details-${cat.replace(/[^a-zA-Z0-9]/g, '-')}`;
            
            htmlContent += `
                <tr class="metadata-row" data-category="${cat}" onclick="document.getElementById('${detailsId}').style.display = document.getElementById('${detailsId}').style.display === 'none' ? 'table-row' : 'none';">
                ...
                <tr id="${detailsId}" class="file-details-row" style="display: none;" onclick="this.style.display = 'none';">
